/* ============================================================
   ФИНАЛЬНОЕ ОБНОВЛЕНИЕ 3.0 — ЦМТО Кунцево PRO
   Все функции в одном файле без дублей
   ============================================================ */

(function() {
    'use strict';

    console.log('🚀 Загрузка финального обновления 3.0...');

    // ============================================================
    // ПРОВЕРКА: не было ли уже инициализации
    // ============================================================
    if (window.__CMTO_UPDATE_LOADED) {
        console.warn('⚠️ Обновление уже загружено, пропускаем');
        return;
    }
    window.__CMTO_UPDATE_LOADED = true;
    ...
    function createProfileSection() {
        // Проверяем, существует ли уже раздел профиля
        ...
        window.verifyUser = function() {
            ...
            if (!key) return showToast('⚠️ Введите ключ верификации');
            if (key.length > 5) {
        ...
        window.generateInternalId = function() {
            if (currentUser.internalId) {
                showToast('❌ ID уже выдан: ' + currentUser.internalId);
                return;
            }
    ...
    function createHotlineDrawer() {
        ...
        window.closeHotline = function() {
            document.getElementById('hotlineDrawer').style.bottom = '-320px';
            document.getElementById('hotlineOverlay').style.display = 'none';
        };
    }

    // ============================================================
    // 5. НОВЫЕ НАСТРОЙКИ (язык, тема, уведомления, liquid glass)
    // ============================================================
    ...
    function createNewSettings() {
        ...
        window.saveSetting = function(setting) {
            if (setting === 'lang') {
                ...
                showToast('🌐 Язык сохранён: ' + (lang === 'ru' ? 'Русский' : 'English'));
                saveState();
            }
        };
    ...
    // ============================================================
    // 7. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (тосты, уведомления)
    // ============================================================
    ...
    setTimeout(() => {
        try {
            ...
            console.log('✅ Финальное обновление 3.0 успешно загружено');
            console.log('📌 Все функции активны: профиль, новости, настройки, горячая линия');
