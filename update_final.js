/* ============================================================
   ФИНАЛЬНОЕ ОБНОВЛЕНИЕ 3.0 — ИСПРАВЛЕННОЕ
   ============================================================ */

(function() {
    'use strict';

    console.log('⏳ update_final.js загружен, ожидаем готовности...');

    function runUpdate() {
        if (typeof DB === 'undefined' || typeof currentUser === 'undefined' || typeof ROLES === 'undefined') {
            console.warn('⚠️ Основные данные ещё не загружены, повторная попытка через 200мс');
            setTimeout(runUpdate, 200);
            return;
        }
        if (typeof ROLE_NAV === 'undefined' || typeof loadTab === 'undefined') {
            console.warn('⚠️ ROLE_NAV или loadTab не определены, повторная попытка через 200мс');
            setTimeout(runUpdate, 200);
            return;
        }
        ...
        function upgradeHeader() {
            ...
            // Определяем функцию toggleBurgerMenu ДО её использования
            const toggleBurgerMenu = function(e) {
                e.stopPropagation();
            ...
            // Делаем toggleBurgerMenu доступным глобально (на случай, если понадобится)
        ...
        setTimeout(() => {
            try {
                ...
                console.log('✅ Финальное обновление 3.0 успешно активировано');
            } catch (e) {
